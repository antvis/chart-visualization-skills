...
export function createRetrieveTool(
  ...
  defaultStrategy: RetrieveOptions["strategy"] = "hybrid",
) {
  return tool({
    description:
      "通过 retrieve 召回最相关参考文档。结果首位自动包含核心约束（使用规则、禁止写法、常见错误）。",
    inputSchema: z.object({
      query: z.string().describe("用户需求或检索关键词"),
      library: z
        .enum(["g2", "g6", "x6"])
        .optional()
        .describe("图表库名称，例如 g2、g6、x6。"),
      topk: z
        .number()
        .int()
        .min(1)
    ...
    execute: async ({ query, library, topk, strategy }) => {
      ...
      console.log(
        `Retrieved ${retrievedSkills.length} skills for query: "${query}" [strategy: ${strategy ?? defaultStrategy}]`,
      );
