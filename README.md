...
// with content: retrieve('bar chart', 'g2', 5, { includeContent: true })
```

```typescript
retrieve(query: string, library?: string, topk?: number, options?: { includeContent?: boolean })
```
...
| `options.includeContent` | `boolean` | `false` | Whether to include full markdown content in each result |
