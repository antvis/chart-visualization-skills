...
console.log('\nSkillRetriever.loadIndex');

test('loads full index', () => {
...
test('loads library-specific index', () => {
  const { indexDir, skillsDir, promptsDir } = createMockIndex();
  const r = new SkillRetriever({ indexDir, skillsDir, promptsDir });
...
test('throws when index file missing', () => {
...
test('returns line chart skill for line query', () => {
  const { indexDir, skillsDir, promptsDir } = createMockIndex();
  const r = new SkillRetriever({ indexDir, skillsDir, promptsDir });
  const results = r.retrieve('line chart');
...
test('filters by library', () => {
  const { indexDir, skillsDir, promptsDir } = createMockIndex();
  const r = new SkillRetriever({ indexDir, skillsDir, promptsDir });
  const results = r.retrieve('chart', { library: 'g6' });
  // all results should belong to g6
  assert.ok(
    results.every((s) => (s.library || (s.skill && s.skill.library)) === 'g6'),
    'all results should be g6'
  );
});

test('respects topK option', () => {
  const { indexDir, skillsDir, promptsDir } = createMockIndex();
  const r = new SkillRetriever({ indexDir, skillsDir, promptsDir });
...
console.log('\nSkillRetriever real index');

test('retrieves from real index without error', () => {
